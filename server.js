...
function validateCookiesText(cookieText) {
  ...
  const youtubeLines = cookieLines.filter(
    (l) => l.includes(".youtube.com") || l.startsWith("youtube.com")
  );

  if (!cookieText.includes("Netscape HTTP Cookie File")) {
    return {
      ok: false,
      message: "Netscape header পাওয়া যায়নি",
      lines: 0,
      loginCookies: false,
    };
  }
  ...
  if (youtubeLines.length < 3) {
    return {
      ok: false,
      message: "YouTube cookie line খুব কম / ভুল format",
...
function setupCookiesFromBase64Variable() {
  ...
  try {
    ...
    addLog("warning", "YT_COOKIES_B64 found but cookies invalid.");
    return null;
  } catch (err) {
...
function getFormatsByQuality(quality) {
  const q = String(quality || "720");

  if (q === "360") {
    return [
      "18/best[height<=360][vcodec!=none][acodec!=none]/best[vcodec!=none][acodec!=none]/best",
      "best[height<=360]/18/best",
    ];
  }

  if (q === "480") {
    return [
      "best[height<=480][vcodec!=none][acodec!=none]/18/best[vcodec!=none][acodec!=none]/best",
      "best[height<=480]/18/best",
    ];
  }

  if (q === "1080") {
    return [
      "best[height<=1080][vcodec!=none][acodec!=none]/22/18/best[vcodec!=none][acodec!=none]/best",
      "best[height<=1080]/22/18/best",
    ];
  }

  return [
    "22/18/best[height<=720][vcodec!=none][acodec!=none]/best[vcodec!=none][acodec!=none]/best",
    "best[height<=720]/22/18/best",
  ];
}

function buildYtdlpArgs(youtubeUrl, quality, client, format) {
  const args = [
    "--no-playlist",
    "--no-warnings",
    "--force-ipv4",
...
async function resolveYoutubeSource(youtubeUrl, quality) {
  ...
  const formats = getFormatsByQuality(quality);
  ...
  for (const client of clients) {
    for (const format of formats) {
      try {
        const label = client ? client : "default";
        addLog("info", `Trying yt-dlp source: client=${label}`);

        const args = buildYtdlpArgs(youtubeUrl, quality, client, format);
        ...
        if (lines.length > 0) {
          addLog("success", `yt-dlp source found with client=${label}`);
          return lines[0];
        }
      } catch (err) {
  ...
  throw new Error(
    "YouTube source পাওয়া যায়নি। Normal public uploaded video link দিন, Live/Shorts/Private link নয়। Last error: " +
      lastError.slice(0, 350)
  );
}
...
async function testCookiesWithVideo(testUrl) {
  ...
  if (!testUrl || !isValidYoutubeUrl(testUrl)) {
    return {
      ok: true,
      message: "Cookies format OK. Video link দিলে real test করা যাবে।",
    };
  }

  await resolveYoutubeSource(testUrl, "360");

  return {
    ok: true,
    message: "Cookies OK. YouTube real test passed.",
  };
}
...
app.post("/api/cookies/check", isLoggedIn, async (req, res) => {
  try {
    ...
    if (!saved.ok) {
      return res.status(400).json({ ok: false, status: cookiesStatus });
    }
    ...
    try {
      ...
      return res.json({ ok: true, status: cookiesStatus });
    } catch (err) {
      cookiesStatus.ok = false;
      cookiesStatus.exists = true;
      cookiesStatus.message =
        "Cookies format OK, কিন্তু YouTube test failed: " + err.message.slice(0, 260);
      cookiesStatus.checkedAt = new Date().toISOString();
